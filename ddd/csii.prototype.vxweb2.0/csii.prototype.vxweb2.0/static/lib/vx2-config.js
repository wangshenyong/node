...
(function(window, vx, $) {
    ...
    function configRemote($remoteProvider) {
         ...
         */
        $remoteProvider.setTrsContext("/store/");
        /**
         *请求回调
         */
    ...
    function configHttpBackend($httpBackendProvider) {
         ...
         */
        $httpBackendProvider.setClientMode();
    ...
    function runRootScope($rootScope, $window, $timeout, $locale, $state, $stateParams, $location, $cookieService, $filter, $remote, $targets, $modal) {
        ...
        $rootScope.$on('$stateChangeStart', function(event, to, pargs, from) {
            // var validateCookie =JSON.parse(decodeURIComponent(document.cookie.slice(9))).flag;
            var UserId = null;
            console.log("onq: " + from.url + '--->' + to.url+"-->"+JSON.stringify(pargs));
            if (!(new RegExp("^(/login)").test())) { //app.xxxx state
                // event.preventDefault();
                console.log("匹配成功 " +from.url + '--->' + to.url);
                console.log(decodeURIComponent(document.cookie))
                // var UserId = $cookieService.getCookie("UserId");
                if (document.cookie) {
                    console.log("触发事件2");
                    UserId =
                        JSON.parse(decodeURIComponent(document.cookie.slice(9)));
                    if (!UserId || UserId == "false") {
                        window.location.href = "http://localhost:8082/#/loginindex/login";
                    }
                } else {
                    console.log("触发事件2");
                     // event.preventDefault();
                    // $rootScope.goto("loginindex.login");
                    window.location.href = "http://localhost:8082/#/loginindex/login";
                }
            } 
